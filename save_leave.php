...
if($years==$year){
  ...
  include "connect.php";
  $strSQL = "SELECT * FROM leave_tbl WHERE username = '".$_SESSION['username']."' and kind_leave = 'ลากิจ'and status = 'approve' and strdate LIKE '%$year%'";
...
$sendmail1 = $objResult4['email'];
 ...
 $strSQL5 = "SELECT * FROM member where username = '".$leader2."'";
  ...
  $reason = $_POST['reason'];
  $strtimes = $_POST['strtime'];
  $lasttimes = $_POST['lasttime'];
  $crtdate = $time;

  $leave_user = 'แจ้งการอนุมัติการลาของ  '.$from;


  $exmail = "From : $from <br>
  เรื่อง : $kind_leave วันที่ : $strdate : $strtimes ถึงวันที่ : $lastdate : $lasttimes <br>
  เหตุผล : $reason <br>
  วันที่ยื่น : $time <br>
...
<?php
  ...
  include "connect.php";
 echo	$strSQL = "SELECT * FROM leave_tbl WHERE username = '".$_SESSION['username']."' and kind_leave = 'ลาป่วย' and status = 'approve' and strdate LIKE '%$year%' ";
  $objQuery  = mysqli_query($objCon,$strSQL);
  while ($objResult= mysqli_fetch_assoc($objQuery))
  {
   $objResult['summary_time'];
...
if($objQuery2)
{
 ...
 echo	$strSQL = "SELECT * FROM leave_tbl WHERE username = '".$_SESSION['username']."' and kind_leave = 'ลาพักผ่อน' and status = 'approve' and strdate LIKE '%$year%'";
  $objQuery  = mysqli_query($objCon,$strSQL);
  while ($objResult= mysqli_fetch_assoc($objQuery))
  {
   $objResult['summary_time'];
